...
public class UrlUtils {
    ...
    public static boolean isMatch(URL consumerUrl, URL providerUrl) {
        ...
        if (!(Constants.ANY_VALUE.equals(consumerInterface) || StringUtils.isEquals(consumerInterface, providerInterface)))
            return false;
        //consumer和provider的category不一样，所以去掉这个限制
...
//        }
    ...
    public static String getServiceCientKey(URL url){
    	return url.getParameter("interface")+"@"+url.getIp()+":"+url.getPort();
    }
    
    public static boolean isMathMethod(URL pattern,String method){
    	return pattern.getParameter(Constants.METHOD_KEY).contains(method);
    }
    
    
    public static InetSocketAddress getAddressFromURlKey(String key) throws Exception{
    	 String[] srv = key.split("@");
         if (srv.length != 2) {
             throw new Exception("invalid service key:" + key);
         }
         String[] hosts = srv[1].split(":");
         if(hosts.length != 2){
        	 throw new Exception("invalid service key:" + key);
         }
